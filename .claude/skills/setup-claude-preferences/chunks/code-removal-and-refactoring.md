...
For in-process code that deploys atomically, update every consumer and delete the old code in the same change. Do not deprecate internal APIs or leave unused compatibility wrappers.
...
Do not rely on source call-site search alone. Check deployment units, transport handlers and schemas, task or event names, persisted data, and version-skew behavior.
...
For direct in-process call sites, count usages before considering a staged cleanup. If there are more than 10–15 consumers and the user explicitly wants a narrow change, ask before choosing deprecation or compatibility scaffolding.

Delete code only after every source consumer and runtime path has been removed.

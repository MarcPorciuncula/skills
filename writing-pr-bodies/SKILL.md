...
- **List preambles** — any clause ending in `:` whose only job is to announce what's about to be listed. *Three failure modes followed: …, There are two reasons: …, A few issues remain: …, Three authoring artifacts: …, The implications are as follows: …, The calibration cycle: …*. Numeric counts, vague quantifiers ("a few", "some", "several"), and label-only headers ("The X:") all count — the disease is the preamble, not the count word. **Structural test:** if the items that follow can stand alone, the preamble is doing nothing — cut it. If it carries information, fold that information into the first item or the surrounding prose. This is a high-frequency LLM tic; treat any colon-then-bullets shape as suspicious by default, even when no count word is present.
...
| "The body has a few issues: the `## Summary` heading, checkboxes under `## Test plan`, and the bot attribution." | "The body uses a `## Summary` heading and checkboxes under `## Test plan`, and ends with a bot attribution." |
| "Three authoring artifacts:\n- A — …\n- B — …\n- C — …" | "- A — …\n- B — …\n- C — …" (drop the preamble; the bullets stand alone) |
| "The calibration cycle:\n- abstract — …\n- expanded page — …" | Either weave both items into prose ("The calibration cycle produced an abstract — … — and an expanded page — …") or drop the preamble and let the bullets carry themselves. |
...
| "Three failure modes followed: …" / "A few issues remain: …" / "Three authoring artifacts: …" / "The calibration cycle: …" / "The change has the following implications: …" | List preamble. The clause exists only to announce the list. Numeric counts, vague quantifiers ("a few", "some", "several"), and label-only headers all count — the disease is the preamble, not the count word. **Structural test:** if the items that follow stand alone, drop the preamble. If it carries information, fold it into the first item. Apply to your own draft: any colon-then-bullets shape is suspicious by default. This is a high-frequency LLM tic and reflexive reach is the failure mode — pattern-match on shape, not just on the example phrases. |
